...
def home_page():
    ...
    if st.checkbox("显示调试信息"):
        st.write(f"当前脚本路径: {os.path.abspath(__file__)}")
        st.write(f"当前脚本所在目录: {current_dir}")
        st.write(f"项目根目录: {root_dir}")
    ...
    with col2:
        ...
        try:
            ...
            # 调试信息
            if st.checkbox("显示图片调试信息"):
                st.write(f"尝试加载图片: {photo_path}")
                st.write(f"图片是否存在: {os.path.exists(photo_path)}")
                if os.path.exists(photo_path):
                    st.write(f"文件大小: {os.path.getsize(photo_path)} 字节")
    ...
    try:
        ...
        # 调试信息
        if st.checkbox("显示简历调试信息"):
            st.write(f"尝试加载简历: {resume_path}")
            st.write(f"简历是否存在: {os.path.exists(resume_path)}")
